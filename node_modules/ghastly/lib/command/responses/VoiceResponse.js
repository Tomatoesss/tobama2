...
exports.VoiceInputMethods = undefined;
 ...
 * @ignore
 */
const VoiceInputMethods = exports.VoiceInputMethods = {
 ...
 */
class VoiceResponse extends _Response2.default {
  /**
  ...
  constructor(context, inputMethod, payload, options = {}) {
    ...
    super(async () => {
      ...
      const isValidInputMethod = Object.values(VoiceInputMethods).filter(method => method === inputMethod).length;

      if (!isValidInputMethod) {
        throw new Error(`Invalid voice input method: ${inputMethod}.`);
      }

      return connection[`play${(0, _string.upperFirst)(inputMethod)}`](payload, options);
    });
  }
}
