...
var _CommandError = require('../errors/CommandError');

var _CommandError2 = _interopRequireDefault(_CommandError);

var _CommandGroup = require('./CommandGroup');

var _CommandGroup2 = _interopRequireDefault(_CommandGroup);

var _CommandObject = require('./CommandObject');

var _CommandObject2 = _interopRequireDefault(_CommandObject);
...
class CommandRegistry {
  ...
  constructor() {
     ...
     * @private
     */
    this.commands = new _StringMap2.default();
     ...
     * @private
     */
    this.aliases = new _StringMap2.default();
     ...
     * @private
     */
    this.groups = new _StringMap2.default();
  }
  ...
  add(...configurators) {
    configurators.map(configurator => configurator({})).forEach(commandConfig => {
      this.addCommand(new _CommandObject2.default(commandConfig));
    });
   ...
   */
  get(identifier) {
    if (!(0, _lang.isString)(identifier)) {
  ...
  applyGroupMiddleware(group, middleware) {
    ...
    if (!commandGroup) {
      throw new _CommandError2.default(`Attempted to apply middleware to non-existent command group: ${group}.`);
    }
   ...
   */
  addCommand(command) {
    if (!(command instanceof _CommandObject2.default)) {
      throw new TypeError('Expected command to be an instance of CommandObject.');
    } else if (this.commands.has(command.name)) {
      throw new _CommandError2.default(`Attempting to add duplicate command: '${command.name}'.`);
    }
    ...
    if (group) {
      if (!this.groups.has(group)) {
        this.groups.set(group, new _CommandGroup2.default(group));
      }
  ...
  alias(name, aliases) {
    ...
    aliases.forEach(alias => {
      if (!(0, _lang.isString)(alias)) {
        throw new TypeError('Expected command alias to be a string.');
      } else if (!this.commands.has(name)) {
        throw new _CommandError2.default(`Attempting to add alias to non-existent command '${name}'.`);
      } else if (this.aliases.has(alias)) {
        throw new _CommandError2.default(`Attempting to add duplicate alias '${alias}' to command '${name}'.`);
      }
